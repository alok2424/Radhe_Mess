...
function getFoodSlot() {
  ...
  if (mins >= LUNCH_START && mins < LUNCH_END)
    return { mode: "SHOW" as const, title: "Today’s LUNCH" };
  if (mins >= DINNER_START && mins < DINNER_END)
    return { mode: "SHOW" as const, title: "Today’s DINNER" };

  if (mins < LUNCH_START)
    return { mode: "HIDE" as const, message: "Your LUNCH will be listed at 11:30am" };
  if (mins >= LUNCH_END && mins < DINNER_START)
    return { mode: "HIDE" as const, message: "Your DINNER will be listed at 6:00pm" };
  return {
    mode: "HIDE" as const,
    message: "Menu is closed for today. Next LUNCH will be listed at 11:30am",
  };
}
    ...
    <section className="space-y-4">
      ...
      {slot.mode === "HIDE" ? (
        <div className="relative overflow-hidden rounded-3xl border border-white/60 bg-gradient-to-br from-orange-50/75 via-amber-50/65 to-orange-100/55 p-5 text-sm text-slate-800 backdrop-blur-xl shadow-[0_26px_70px_-50px_rgba(15,23,42,0.65),0_22px_56px_-46px_rgba(249,115,22,0.5)]">
          ...
          <div className="relative text-[15px] font-medium tracking-wide">
            {slot.message}
          </div>
        </div>
      ) : (
        <>
          <div className="inline-flex w-fit items-center gap-2 rounded-3xl border border-white/60 bg-gradient-to-r from-orange-50/70 via-amber-50/70 to-orange-100/55 px-4 py-2 text-base font-semibold tracking-wide text-slate-900 backdrop-blur-xl shadow-[0_22px_60px_-44px_rgba(15,23,42,0.55),0_18px_44px_-36px_rgba(249,115,22,0.45)]">
          ...
          {loading ? (
            <div className="relative overflow-hidden rounded-3xl border border-white/60 bg-gradient-to-br from-orange-50/70 via-amber-50/60 to-orange-100/50 p-5 text-sm text-slate-800 backdrop-blur-xl shadow-[0_22px_56px_-44px_rgba(15,23,42,0.6),0_18px_44px_-38px_rgba(249,115,22,0.45)]">
              <div className="text-[15px] font-medium tracking-wide">Loading menu...</div>
            </div>
          ) : items.length === 0 ? (
            <div className="relative overflow-hidden rounded-3xl border border-white/60 bg-gradient-to-br from-orange-50/75 via-amber-50/65 to-orange-100/55 p-5 text-sm text-slate-800 backdrop-blur-xl shadow-[0_26px_70px_-50px_rgba(15,23,42,0.65),0_22px_56px_-46px_rgba(249,115,22,0.5)]">
